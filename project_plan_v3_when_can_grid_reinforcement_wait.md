...
- **Concrete, decisive build:** primary grid = a **SimBench MV grid (semi-urban/urban) with LV aggregated at secondary substations** via the `simbench` 1.6.1 + pandapower stack; **drop the parameter-less Liander grid** for this paper; physics = **two-tier** (net-load summation in the Monte Carlo loop + `lightsim2grid` AC validation subset — native pandapower solves take ~28–37 ms/step, lightsim2grid ~0.17–0.6 ms, ~20× faster); focus on **critical winter-week windows**; N = 1e4–1e5 with common random numbers; **P_crit ≈ 1e-2** (1e-3 sensitivity), justified by IEC 60076-7 thermal tolerance and observed <1% DSO exceedance practice, with the honest caveat that no single official Dutch loading-% exists.
...
**C. TIME STRUCTURE — critical-period focus.** Do *not* run full-year 15-min in the MC inner loop for headline results. Define the overload metric **per planning year over a critical-period window** — winter cold-spell weeks driving combined HP+EV evening peaks (the binding demand-driven regime). Use SimBench full-year series to *identify/extract* the critical weeks, then Monte-Carlo the aleatory layer within them. Run the full-year 15-min pass once per scenario as a Tier-2 check that the window captures annual max-loading exceedances and to screen the summer reverse-PV-flow regime.
...
**Python 3.12**, **pandapower 3.x** (`pandapower.networks` for CIGRE/IEEE-LV), **simbench 1.6.1** (`sb.get_simbench_net`), **lightsim2grid ≥0.9.2** (C++ NR backend), **numba** JIT on, **numpy/pandas/scipy**. The p-box/possibility layer is implemented in-house (thin α-cut propagation, a few hundred lines) with an optional cross-check against an established probability-bounds library. **Code + data availability statement** ready for Applied Energy: public repo, pinned `environment.yml`, SimBench/ElaadNL/When2Heat retrieval scripts (data redistributed only where licences permit; otherwise download scripts + DOIs).
