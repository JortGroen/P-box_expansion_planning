...
- Repeated mechanical work must be automated before it is scaled. If a task repeats more than about three times, or involves batches of API calls, downloads, profile generation, checksums, artifact rebuilds, scenario sweeps, or manifest validation, write or extend a deterministic script/runner with tests, a small pilot, and resumable checkpoints instead of manually babysitting the loop.
- Fast stacked integration is allowed only when it fails closed. Any executable integrated run must stop on unsigned values, proposed-only assumptions, missing source artifacts, unresolved blocker IDs, unsafe placeholders, or disallowed tokens (`TODO`, `TBD`, `placeholder`, `synthetic`, `proposed`, `unsigned`, `not-approved`) unless those tokens are explicitly allowlisted as non-executable metadata. Integration PRs must produce a concise blocker manifest for PI review before any scientific number is accepted.
...
- Per PV-ORIENT-001, first-experiment PV orientation/tilt uses a typical/statistical distribution only. Do not implement per-building, per-roof, location-specific, 3DBAG, or PV-map geometry extraction/allocation in the first-experiment path. Exact distribution source, weights, bins, and conversion treatment remain PI-gated before executable PV use.
...
- Per G0-A3, the primary executable event is strictly `L_import > 1.0 p.u.` for four consecutive 15-minute import steps over the full planning year. Persistent-event sensitivities at `1.1` and `1.2 p.u.` are predeclared; no separate 1.0-1.1 cumulative-exposure rule is primary. Q-5 is resolved, but all other gates and unsigned values still apply.
...
- Per A-016, before executable integrated analysis, manifests must expose EV/HP/PV source lineage, source year, planning year, scenario label, scaling/adoption branch, and a scenario-consistency check across ElaadNL, PBL/CBS, CBS PV, and II3050 inputs. Do not silently mix inconsistent `low`/`middle`/`high` meanings; report the mismatch or stop for PI decision.
