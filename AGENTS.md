...
- PI-dashboard assistance uses the lightweight protocol in `reports/PI_DASHBOARD_CACHE.md`: start from the cache, live PR/status summaries, and targeted register searches; open full registers, plans, logs, or CI output only when a decision, review, or discrepancy requires it. This reduces Codex usage without changing document authority.
