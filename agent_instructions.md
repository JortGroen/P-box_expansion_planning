...
- **First-experiment PV geometry scope (PV-ORIENT-001):** represent PV orientation and tilt through a signed typical/statistical distribution only. Do not implement per-building, per-roof, location-specific, 3DBAG, or PV-map geometry extraction/allocation in the first-experiment path. Exact distribution source, bin definitions, weights, and whether/how the distribution changes PV conversion remain PI-gated before executable PV use; the full roof-level workflow is deferred until after the first real experiment.
