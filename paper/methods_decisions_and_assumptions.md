...
| PV-CAP-001/D-014 | PV installed capacity is anchored to local Alkmaar CBS photovoltaic capacity and scaled to 2035 through a signed II3050/scenario growth factor; D-014 now proposes exact source/schema approval keys plus a lightweight statistical orientation/tilt route for first-experiment review. | Approved route; packets proposed; concrete retrieval, values, capacity convention, growth factor, class weights, conversion, and per-node allocation remain pending. |
...
**Status: Proposed source/value packet; retrieval/checksum and numeric values pending.** D-014 records the public-source bundle needed to implement PV-CAP-001. The primary candidate local anchor is CBS StatLine/OData table 85005NED for Alkmaar `GM0361`, because it is public, regionally disaggregated, and reports installed PV capacity and installation counts by sector and size class. The D-014 source/value packet now records exact schema probes and row-filter templates, but the source period, selected field, units, and DC/AC convention remain unsigned until raw retrieval and schema inspection. A separate statistical orientation/tilt packet proposes first-experiment class-based geometry handling only; it explicitly defers building-level and roof-level extraction such as 3DBAG per-roof processing. Netbeheer Nederland II3050 edition 2 supplies the candidate 2035 PV scenario-growth source, but the scenario column and growth-factor value remain unsigned. 3DBAG, DEGO, CBS building-geography tables, and Zonnedakje may support later sensitivity, validation, source discovery, or spatial allocation only after their concrete Alkmaar data, license, retrieval path, and provenance are registered; they are not first-experiment building-level geometry inputs. Before executable PV input uses this bundle, the project must record the exact retrieved files or API queries, checksums, selected geography, selected source year, selected capacity field and DC/AC convention, II3050 growth factor, per-node allocation rule, and signed PV-PARAM conversion decision. D-014 does not approve any installed-capacity number, allocation, net-load/event analysis, `P(E)`, capacity-screen result, or manuscript number.
<!-- methods-id: D-013 -->
