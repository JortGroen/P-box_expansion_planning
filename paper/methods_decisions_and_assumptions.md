...
<!-- methods-id: E2-S3-HP001-COMPONENT-OUTPUT-RUNNER-READINESS -->
### E2-S3-HP001-COMPONENT-OUTPUT-RUNNER-READINESS - HP-001 Component-Output Runner/Preflight Scaffold

**Status: Proposed runner scaffold; real HP component outputs blocked.** The HP-001 component-output runner consumes the component-output readiness manifest before any HP artifact can be handed to IC-1. It writes a deterministic blocker/output manifest with request identity, input path/size/SHA-256, blocker IDs, validator names, git/code identity, and non-claims. The runner reuses the HP readiness validator, adds concrete D-003/D-004/D-013 source artifact path/checksum/provenance requirements, rejects stale placeholder approval strings or template path/status tokens, verifies exact HP/PV WEATHER-001 identity equality, preserves the four SFH/MFH space/DHW component traces, blocks premature accepted-artifact status claims, and refuses real component-output generation in the current scaffold. A signed synthetic-fixture branch may write a four-step IC-1-compatible NPZ/manifest with positive HP `p_kw`, zero `q_kvar`, `timestamps`, and shared weather identity metadata for loader tests only. This runner does not approve annual HP TWh values, 2035 adoption/electrification/service fractions, final D-004 paired-weather or cold-spell acceptance, real HP profile or component-output generation, net-load/event analysis, `P(E)`, threshold/capacity-screen results, manuscript numbers, or probability results.
