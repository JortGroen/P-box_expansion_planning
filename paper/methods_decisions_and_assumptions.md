...
| D014-PV-FIRST-EXPERIMENT-VALUE-DECISION-PACKET | Proposed first-experiment value-decision packet narrows concrete PI choices for statistical orientation/tilt, conversion, losses, temperature/clipping, capacity convention, node allocation, and A-016 before executable PV. | Proposed packet; values and executable PV blocked. |
| D014-PV-FIRST-EXPERIMENT-VALUE-APPROVAL-PACKET | Proposed first-experiment value-approval packet turns the remaining PV capacity, II3050/A-016, statistical orientation/tilt, PV conversion, reactive-power, manifest path/checksum, and node-allocation choices into one explicit PI signature checklist. | Proposed packet; values and executable PV blocked. |
| D014-PV-COMPONENT-OUTPUT-ARTIFACT-SCAFFOLD | Proposed PV component-output scaffold records the future IC-1-compatible manifest/NPZ schema, checksums, weather identity, path/checkpoint policy, and blocker behavior before signed PV values exist. | Proposed scaffold; component-output generation blocked. |
| D014-PV-COMPONENT-OUTPUT-RUNNER-PREFLIGHT | Proposed PV component-output runner/preflight requires signed D-014 capacity/growth, statistical orientation/tilt, PV conversion, allocation, reactive-power/path, A-016, and final paired HP/PV artifacts before real PV arrays can be emitted; current main produces blocker manifests only. | Proposed runner/preflight; real PV output blocked. |
| PV-CAP-001/D-014 | PV installed capacity is anchored to local Alkmaar CBS photovoltaic capacity and scaled to 2035 through a signed II3050/scenario growth factor; D-014 now has retrieved CBS Alkmaar source evidence, retrieved II3050 appendix evidence, an unsigned capacity value-choice packet, an unsigned capacity approval-template packet, an unsigned executable-readiness blocker packet, an unsigned executable preflight guard packet, an unsigned PV-PARAM conversion source-choice packet, an unsigned first-experiment approval packet, an unsigned first-experiment value-approval packet, an unsigned PV component-output runner preflight, and proposed lightweight statistical orientation/tilt packets, while heavy 3DBAG/roof-level geometry is deferred until after the first real experiment. | Approved route; CBS/II3050/value-choice/template/blocker/preflight/conversion-choice/first-experiment approval/value-approval evidence proposed; executable capacity value, capacity convention, II3050 scenario/growth factor, A-016 consistency, per-node allocation, statistical orientation/tilt source, weights, bins, and conversion treatment remain pending. |
...
| E5.S4 | The math-core trust certificate requires both the analytic Gaussian cross-check and a published hybrid-propagation reproduction. | The analytic synthetic fixture now records tolerance, alpha-row nestedness, separate lower/upper CI containment, and no-defuzzification guards in `e5s4-math-core-trust-certificate-v1`; the published Baudrit-style reproduction remains fail-closed until verified source/example provenance IDs and reproduction evidence are approved and reproduced, and G3 remains pending for paper-facing vertex shortcut claims. |
| E3-S2B-PRERUN-GATE | Proposed A-owned pre-run gate requires checksum-backed capacity provenance, accepted loadable component-output manifests, A-016 scenario consistency, and explicit blockers before E3.S2b launch. Multi-node component-output wrappers are blocked until the IC-1 loader contract supports them or per-node manifests exist. | Proposed scaffold protocol; no screen/result authority. |
...
The E4.S1 dense-rho sweep scaffold remains synthetic-only before G3. Its serialized payload is versioned as `e4s1-synthetic-rho-sweep-v1` and carries an explicit pending-G3 status plus non-claims for real trajectories, real `P(E)`, capacity screening, vertex-shortcut authorization, and manuscript numbers. The validator rejects paper-facing relabeling and collapsed probability fields so the synthetic diagnostic cannot become a paper-facing result by serialization alone. The companion `e4s1-alpha-event-count-estimator-v1` scaffold accepts only precomputed lower and upper endpoint event counts by alpha, recomputes probability estimates and confidence intervals from those counts, requires common sample identities and endpoint metadata, and emits only alpha-indexed lower/upper rows plus real-use blocker keys. It does not authorize real event probabilities before real endpoint manifests, signed G2 and A-013 inputs, capacity convention/provenance, A-016 consistency, and any required G3 vertex-shortcut approval are present.
...
<!-- methods-id: D014-PV-FIRST-EXPERIMENT-VALUE-DECISION-PACKET -->
### D014-PV-FIRST-EXPERIMENT-VALUE-DECISION-PACKET - First-Experiment PV Value-Decision Packet

**Status: Proposed packet; values and executable PV blocked.** This packet converts the merged first-experiment approval surface into a concise value-decision checklist for PI review. It consumes checksummed metadata for the D-014 capacity value-choice packet, the PV-ORIENT-001 orientation/tilt value-choice packet, the PV-PARAM conversion source-choice packet, the first-experiment approval packet, and the executable preflight guard. It recommends, for review only, a lightweight route in which a later signed D-014/PV-CAP capacity artifact is paired with a later signed statistical orientation/tilt class distribution and a later signed conversion treatment, preferably a traceable statistical plane-of-array route if decomposition, transposition, albedo, losses, temperature, and clipping are approved. Direct KNMI-GHI scalar conversion remains only an explicit fallback if the PI signs that simplification. The packet also makes capacity convention, node allocation, A-016 scenario consistency, PVGIS qualitative-only provenance, and final paired HP/PV acceptance separate blockers. It does not approve any PV capacity value, II3050 growth factor, capacity convention, orientation/tilt bin or weight, performance ratio, loss value, temperature coefficient, clipping rule, conversion formula, allocation, PV output, net-load/event analysis, `P(E)`, threshold analysis, capacity screen, manuscript number, roof/building/3DBAG/PV-map geometry, or final paired HP/PV acceptance.
<!-- methods-id: D014-PV-FIRST-EXPERIMENT-VALUE-APPROVAL-PACKET -->
### D014-PV-FIRST-EXPERIMENT-VALUE-APPROVAL-PACKET - First-Experiment PV Value-Approval Packet

**Status: Proposed packet; values and executable PV blocked.** This packet is the compact PI signature checklist for turning the first-experiment PV route from proposed metadata into later executable values. It consumes checksummed metadata for the D-014 value-decision packet, capacity value-choice packet, capacity approval-template packet, PV-ORIENT-001 orientation/tilt value-choice packet, PV-PARAM conversion source-choice packet, and PV component-output scaffold. It records the exact remaining choices for the Alkmaar CBS 85005NED `GM0361` capacity anchor row, period, sector/category, capacity field, and DC/AC convention; the II3050 2035 scenario column, denominator, growth formula, and A-016 scenario-consistency link; the statistical orientation/tilt source, bins, representative angles, weights, and capacity-weighting convention; the PV conversion route, losses, temperature, and clipping treatment; the PV reactive-power policy; repository-relative manifest path/checksum policy; node allocation and normalization; and final paired HP/PV acceptance. Until those keys are signed in successor artifacts, future executable PV generation must fail closed and return only a blocker manifest. This packet keeps KNMI as the realized WEATHER-001 path, PVGIS as qualitative sanity/provenance only, and roof/building/location-specific 3DBAG or PV-map geometry deferred until after the first experiment. It does not approve any PV capacity value, II3050 growth factor, capacity convention, orientation/tilt bin or weight, PR/loss/temperature/clipping value, conversion formula, reactive-power convention, component-output path policy, allocation, PV output, net-load/event analysis, `P(E)`, threshold analysis, capacity screen, manuscript number, or final paired HP/PV acceptance.
<!-- methods-id: D014-PV-COMPONENT-OUTPUT-ARTIFACT-SCAFFOLD -->
### D014-PV-COMPONENT-OUTPUT-ARTIFACT-SCAFFOLD - PV Component-Output Artifact Scaffold

**Status: Proposed scaffold; component-output generation blocked.** This scaffold prepares the PV-owned side of future first-experiment component-output generation without emitting real PV arrays. It consumes the current checksummed D-014 value-decision packet, executable preflight guard, and accepted D-004 weather input artifact, then records the future IC-1-compatible manifest and NPZ shape: `kind = pv`, node/member/source/calendar identity, `shared_weather_driver_id`, repository-relative array path, array SHA-256, `p_kw`, `q_kvar`, timestamps, and provenance. The sign convention follows IC-1: PV export is represented as negative `p_kw`, while nonnegative `generation_kw` remains an internal PV diagnostic. The reactive-power policy remains unsigned unless the PI later signs either a `q_kvar = 0` first-experiment convention or another PV reactive-power treatment. Current committed D-014/PV packets are still rejected because signed capacity, statistical orientation/tilt values, PV-PARAM conversion/loss/temperature/clipping treatment, node allocation, A-016 scenario consistency, final paired HP/PV acceptance, reactive-power policy, and path/checksum policy are missing. Synthetic-fixture output artifacts may be written in tests only when explicitly marked `synthetic_fixture`. Accepted executable specs must reject stale/template approval strings, including `future`, `pending`, `todo`, `tbd`, `not-approved`, and `<...>` placeholders, and must use non-empty repository-relative component-output array/manifest paths with no absolute/rooted path or `..` segments. This scaffold does not approve PV capacity, growth factor, orientation/tilt weights, conversion formula, losses, temperature coefficients, clipping rules, allocation, real PV output, net-load/event analysis, `P(E)`, threshold analysis, capacity screens, manuscript numbers, roof/building/3DBAG/PV-map geometry, or final paired HP/PV acceptance.
<!-- methods-id: D014-PV-COMPONENT-OUTPUT-RUNNER-PREFLIGHT -->
### D014-PV-COMPONENT-OUTPUT-RUNNER-PREFLIGHT - PV Component-Output Runner Preflight

**Status: Proposed runner/preflight; real PV output blocked.** The PV component-output runner/preflight is the PV-owned automation boundary that will later emit IC-1-compatible PV component-output artifacts only after all first-experiment PV value artifacts are signed. It consumes the proposed D-014 value-approval packet and component-output scaffold and requires checksum-backed signed artifacts for capacity and II3050 growth, statistical orientation/tilt distribution, PV conversion/loss/temperature/clipping treatment, node allocation and normalization, reactive-power policy, repository-relative manifest path/checksum policy, A-016 scenario consistency, and final paired HP/PV weather acceptance. With current committed inputs those approvals are absent, so the runner returns a blocker manifest with no arrays and no component-output manifest. Synthetic-fixture output is allowed only in tests with explicit fixture status and provenance and is validated through the IC-1 loader; it is not PV scientific output. The preflight rejects stale/template approval tokens, missing source/checksum metadata, unsafe paths, WEATHER-001 identity mismatches, missing allocation, premature accepted status, and roof/building/3DBAG/PV-map geometry hooks in the first-experiment path. It does not approve PV capacity, II3050 growth, orientation/tilt values, conversion formulas, losses, temperature or clipping values, reactive-power policy, allocation, real PV output, net-load/event analysis, `P(E)`, threshold analysis, capacity screens, manuscript numbers, or final paired HP/PV acceptance.
...
**Status: Proposed preflight template and runner scaffold; executable HP artifacts unsigned.** The HP-001 profile artifact rebuild/checksum preflight defines the metadata that a future rebuild request must satisfy before any HP profile artifact generation can run. The proposed guard requires signed approval IDs for annual value binding, 2035 adoption/electrification, A-016 scenario-source consistency, D-004 paired-weather acceptance, and cold-spell tolerances, and rejects template or stale approval strings such as `<future ...>`, `placeholder`, `proposed`, `pending`, `unsigned`, `TODO`, `TBD`, or `not-approved`; source artifact records for D-003 When2Heat, D-004 WEATHER-001 member input, and D-013 value binding with path, SHA-256, data ID, and provenance; identical HP/PV WEATHER-001 identity fields; 35,040 quarter-hour output targets; checksum-manifest targets; and an empty unresolved-blocker list. The metadata-only runner in `data/get_hp_scaling.py --run-profile-rebuild-preflight` consumes either the committed template packet or a future direct preflight manifest and writes a blocker/output manifest with request identity, input path/size/SHA-256, blocker IDs, validator name, git/code identity, and non-claims. The committed JSON output is intentionally blocked because the template contains placeholder approvals, missing source checksums, and non-matching placeholder HP/PV weather identities; a signed synthetic fixture can only reach an accepted handoff shape and still performs no profile generation. This packet does not approve annual HP TWh values, 2035 adoption/electrification/service fractions, final D-004 paired-weather or cold-spell acceptance, a real profile rebuild, net-load/event analysis, `P(E)`, threshold/capacity-screen results, manuscript numbers, or probability results.
...
<!-- methods-id: E3-S2B-PRERUN-GATE -->
### E3-S2B-PRERUN-GATE - E3.S2b Pre-Run Gate Scaffold

**Status: Proposed scaffold protocol; executable screen authority unsigned.** The E3.S2b pre-run gate is a metadata-only fail-closed boundary for the future capacity/domain screen. It requires a checksum-backed capacity-provenance packet for the decision-transformer bank, preserves the explicit pending total-versus-firm convention status, records G0-A3 threshold semantics only as metadata, and requires accepted component-output manifests for baseline, EV, HP, PV, adoption, and flexibility before any real screen can launch. A component-output manifest is not executable merely because it exists: its node-axis and array-shape contract must match the current IC-1 loader. In the current scaffold, that loader accepts one single-node, one-dimensional NPZ time series per manifest, so multi-node scenario arrays or pseudo-node wrappers are reported as blockers until Agent A adds an explicit multi-node adapter or Agent C emits per-node loadable manifests. The gate also keeps A-016 scenario-consistency evidence, A-013 grid-error values, G2 Tier-1 endpoints, capacity-convention choice, and missing checksums as named blockers. This proposal does not run IC-1 aggregation, IC-2, event detection, P(E), a capacity/domain screen, denominator selection, or manuscript-number production.
<!-- methods-id: OWN-001 -->
