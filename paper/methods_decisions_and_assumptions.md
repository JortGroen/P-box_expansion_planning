...
| PV-CAP-001/D-014 | PV installed capacity is anchored to local Alkmaar CBS photovoltaic capacity and scaled to 2035 through a signed II3050/scenario growth factor; D-014 now proposes exact source/schema approval keys plus a lightweight statistical orientation/tilt packet, while heavy 3DBAG/roof-level geometry is deferred until after the first real experiment. | Approved route; D-014 packets proposed; concrete retrieval, values, capacity convention, growth factor, per-node allocation, statistical orientation/tilt source, weights, bins, and conversion treatment remain pending. |
