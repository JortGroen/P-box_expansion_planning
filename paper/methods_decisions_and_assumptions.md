...
| PV-CAP-001/D-014 | PV installed capacity is anchored to local Alkmaar CBS photovoltaic capacity and scaled to 2035 through a signed II3050/scenario growth factor; the D-014 packet now proposes exact source/schema and approval-key choices, while heavy 3DBAG/roof-level geometry is deferred until after the first real experiment. | Approved route; D-014 packet proposed; concrete retrieval, values, capacity convention, growth factor, per-node allocation, and statistical orientation/tilt values remain pending. |
| PV-ORIENT-001 | For the first real experiment, PV orientation and tilt are represented through a typical/statistical distribution rather than per-building, per-roof, or location-specific geometry. | Approved scope; exact distribution source, weights, bins, and conversion treatment remain pending. |
