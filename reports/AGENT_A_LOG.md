...
DID: Started fresh branch `agent-a/E1.S2b-timeseriescpp-benchmark` from latest `origin/main`. Added a TimeSeriesCPP adapter and benchmark runner for the selected SimBench primary grid, with topology materialization for LightSim conversion, input-shape checks, raw timing output, a benchmark report, and an evidence manifest. Updated the G1 methods paragraph to preserve the approved-with-conditions status while reflecting the corrected compute-path evidence.
VERIFIED: Pre-change `.\scripts\task.ps1 test` passed: 43 tests. Focused `.\.venv\Scripts\python.exe -m pytest tests\test_evaluator_ac_benchmark.py` passed: 3 tests. Post-change `.\scripts\task.ps1 test` passed: 46 tests. Manifest `reports/benchmark_timeseriescpp_evidence.json` records source/config commit `acb9f58e3e650f554fa904a935b70633f380e3f5`, package versions, command, hardware/runtime context, and checksums for `reports/BENCHMARK_TIMESERIESCPP.md` and `reports/benchmark_timeseriescpp_raw.json`.
OPEN: TimeSeriesCPP solved 672 repeated baseline steps in every measured repeat. Median internal solver time was 119.995 ms per 672-step batch (0.1786 ms/step); median `compute_Vs` wall time was 125.407 ms per batch (0.1866 ms/step). High-level `runpp(lightsim2grid=True)` recorded `net._options['lightsim2grid'] == True` in every measured run, converged without fallback warnings, and had median wall time 103.823 ms per single solve. The corrected voltage-solve-only full-year AC validation budget is approximately 6.54 s for 35,040 deterministic steps before scenario construction and selected result extraction. No G1, G2, event-definition, A-013, epsilon-grid, transformer denominator, or IC schema decision was changed.
