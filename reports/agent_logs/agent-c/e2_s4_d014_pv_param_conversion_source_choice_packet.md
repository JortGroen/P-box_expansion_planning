...
VERIFIED: Focused PV/data-source/methods tests passed with 130 passed; `scripts/task.ps1 ownership` passed; `git diff --check` passed; `scripts/task.ps1 test-fast` passed with 688 passed, 1 skipped, and 7 deselected.`r`nNEXT: Open a normal PR if the latest-main merge and final validation stay green.