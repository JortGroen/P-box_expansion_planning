...
This certificate is a runner/report readiness surface rather than a scientific result. It is intended to fail closed if the analytic p-box path drifts outside tolerance, if the payload is relabeled as paper-facing, or if serialized rows are tampered after JSON transport.
...
- `build_gaussian_crosscheck_manifest` emits a JSON-stable analytic certificate with alpha-indexed oracle/estimate rows, absolute errors, a tolerance guard, and explicit synthetic-only non-claims; `assert_gaussian_crosscheck_manifest_payload` validates the serialized payload.
