...
For 672 repeated baseline steps, TimeSeriesCPP reports a median internal solver time of 149.372 ms total (0.2223 ms/step). The measured `compute_Vs` Python wall time is 156.216 ms total (0.2325 ms/step). This supports an AC validation budget on the order of one full-year deterministic trajectory in 8.15 s for voltage solves alone, before scenario construction and selected result extraction.
...
- Timestamp: `2026-07-14T15:33:39.361441Z`

## Timing Summary

| Component | Median ms | Notes |
| --- | ---: | --- |
| Network conversion/setup | 4854.669 | load grid, materialize topology, pandapower baseline solve, LightSim conversion, TimeSeriesCPP construction |
| Time-series input update | 1.276 | construct repeated `gen_p`, `sgen_p`, `load_p`, `load_q`, and `vinit` arrays |
| `compute_Vs` wall time | 156.216 | TimeSeriesCPP voltage solve for 672 steps |
| Internal solver time | 149.372 | Reported by TimeSeriesCPP |
| Internal preprocessing time | 2.920 | Reported by TimeSeriesCPP |
| Voltage result extraction | 0.027 | `get_voltages()` to NumPy |
| Current-flow extraction | 5.561 | `compute_flows()` and `get_flows()` |
| Active-power-flow extraction | 3.464 | `compute_power_flows()` and `get_power_flows()` |

## High-Level `runpp` Diagnosis

The high-level pandapower benchmark accepted `lightsim2grid=True`: every measured run recorded `net._options['lightsim2grid'] == True`, converged, and emitted no fallback warnings. Its median wall time was 42.185 ms per solve, versus 50.149 ms for native pandapower on the same grid. Therefore the prior result should be interpreted as high-level pandapower orchestration plus LightSim-compatible Newton solve, not as the standalone numerical cost of the lower-level C++ time-series path.
