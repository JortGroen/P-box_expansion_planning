...
| PV-PARAM-001 | 2026-07-23 | PV conversion parameter signoff | Proposed: require PI signoff before PV conversion parameters are used for signed executable PV input. The proposed packet covers installed-capacity handling, tilt/aspect and plane-of-array treatment, losses/performance ratio, temperature coefficient/reference temperature, clipping, and whether KNMI Q-derived GHI may be used directly or must be transformed. Until signed, `PVSystemConfig` remains an unsigned/fail-closed scaffold for tests and review only. | Scientific PV conversion values are not implied by D-004 source/member acceptance or by the normalized PVGIS reference request. | `data/metadata/weather_pv/d004_pv_parameter_decision_packet.json`; `reports/e2_s4_pv_readiness_wording_and_parameters.md`. | proposed | pending PI review |
