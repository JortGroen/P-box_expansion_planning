...
| E2-S3-HP-LOCAL-SCALING-SOURCE-USE-PROPOSAL | 2026-07-22 | HP-001 D-013 local scaling source-use proposal | Proposed: use the D-013 schema inspection to select candidate PBL Startanalyse 2025 Alkmaar residential heat-demand indicators and CBS Alkmaar SFH/MFH denominator evidence for later HP-001 annual thermal scaling. The proposal identifies `H23_Vraag_RV_w` and `H24_Vraag_TW_w` in `Alkmaar_strategie.csv`, unit `[GJ/weq/jaar]`, with `Referentie_2030` as the candidate heat-demand value column; proposes PBL `I11_woningequivalenten [Woning]` as the intensity denominator; and proposes CBS 85035NED `2026JJ00` `Eengezinswoningen totaal`/`Meergezinswoningen totaal` as the SFH/MFH crosswalk. It records unsigned illustrative component values before any 2035 HP adoption multiplier. | This turns the D-013 schema evidence into a reviewable source-use route without making values executable. It preserves HP-001 space/DHW separation by mapping `H23` to space and `H24` to domestic hot water, keeps SFH/MFH allocation as a signed split-rule decision, and keeps 2035 HP adoption/electrification separate from local heat-demand evidence. This row does not sign PBL columns, unit conversions, class split rules, annual TWh values, adoption/service fractions, D-004 acceptance, cold-spell tolerances, net-load/event/`P(E)`/threshold/capacity-screen analysis, manuscript numbers, or probability results. | `reports/e2_s3_hp_local_scaling_source_use_proposal.md`; `data/metadata/hp_scaling/hp001_alkmaar_gm0361_local_scaling_source_use_proposal.json`; D-013; HP-001; `reports/e2_s3_hp_scaling_schema_inspection.md`. | proposed packet; values unsigned | -- |
...
| EV-005B | 2026-07-22 | Within-realization EV replacement policy packet | Proposed: choose an executable EV member-selection rule before IC-1 materializes source-member draws. Agent C recommends charge-point-level sampling with replacement from the verified candidate library for each component and EV-008A public capacity class, recording duplicate selections as explicit bootstrap multiplicities with RNG-001 component-stream and source-member provenance. Whole-grid no-replacement is marked infeasible for approved 2035 Alkmaar counts because home K and public per-class K exceed candidate M; node-local no-replacement with cross-node reuse remains an unsigned alternative. | Makes the previously pending EV-005 replacement rule concrete for PI review without loading profiles or inspecting held-out adequacy. It separates executable conditional sampling from finite-library adequacy, which still requires downstream E3.S2a evaluation and does not certify M sufficiency. | `reports/e2_s2_ev005_replacement_policy_packet.md`; `data/metadata/ev_adoption/e2_s2_ev005_replacement_policy_packet.json`; EV-003; EV-005; EV-007A; EV-008A; EV-CAL-001; RNG-001. | proposed packet; PI decision required before implementation | -- |
