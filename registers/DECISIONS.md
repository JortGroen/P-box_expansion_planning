...
| EV-007 | 2026-07-21 | Proposed local EV adoption count basis | Proposed for PI review: use Option A from Q-7 by selecting a representative CBS-area cluster before integrated congestion results are inspected, then deriving 2035 home and public charge-point totals from ElaadNL Outlook local forecast outputs for that cluster. This PR proposes Lingewaard (`GM1705`) as a municipality-level CBS-area cluster because the live neighbourhood-list proxy returned HTTP 500 during the session while the municipality local forecast endpoint was accessible. The resulting values remain proposed and may not drive `adoption_scenarios`, nodal allocation, EV-005 replacement decisions, or integrated runs until the PI signs EV-007/Q-7. | Provides an auditable local-count route without treating D-010 national totals as SimBench physical counts. Using a predeclared local cluster preserves the Option A logic while retaining the neighbourhood-access failure as a limitation rather than silently falling back to national scaling. | `data/metadata/ev_adoption/e2_s6_local_adoption_counts_metadata.json`; `reports/e2_s6_local_adoption_counts.md`; Q-7; A-014; D-010. | proposed; pending PI sign-off | -- |
