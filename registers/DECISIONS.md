...
| E2-S3-COLD-SPELL-ACCEPTANCE-DESIGN | 2026-07-21 | HP cold-spell and paired-weather acceptance design | Proposed: before D-003 is signed for HP source use, a future acceptance report should verify that When2Heat-derived HP profiles and PV profiles preserve the same shared weather realization, canonical UTC/local calendar, source/provenance, and paired temperature/irradiance fields, then report predeclared cold-spell and temperature-response diagnostics. Numerical pass/fail tolerances remain unsigned and must be approved before the first real acceptance run. | Keeps the D-003 source-use decision separate from parser readiness and prevents HP/PV weather consistency from being judged after integrated congestion results are inspected. This row does not sign D-003, set tolerances, run the check, or authorize net-load, event, `P(E)`, capacity-screen, manuscript, or probability results. | `reports/e2_s3_cold_spell_acceptance_design.md`; Q-8 remains open; D-003 and D-004 remain proposed. | proposed | -- |
| E2-S3-HP-TECH-SCALING-DECISION-PACKET | 2026-07-21 | HP technology and annual scaling decision packet | Proposed: the PI review packet frames the remaining E2.S3 choices before real HP integration: Dutch When2Heat shape columns, COP/technology columns, optional water-heating inclusion, SFH/MFH/COM class handling, and whether annual thermal scaling comes from When2Heat `heat_demand_*` evidence or from another registered source. It proposes review options only and leaves all scientific choices unsigned. | Collects previously separate HP source-use, annual-scaling, and cold-spell blockers into one PI-facing decision map so D-003 source use is not silently conflated with technology scenario, annual volume, local downscaling, or paired-weather acceptance. This row does not sign D-003/D-004, set numerical tolerances, run paired-weather acceptance, authorize net-load/event/`P(E)`/capacity/manuscript/probability results, or approve any final HP integration input. | `reports/e2_s3_hp_technology_scaling_decision_packet.md`; D-003 and D-004 remain proposed; Q-8 remains open. | proposed | -- |
...
| EV-008 | 2026-07-21 | Proposed public charge-point profile protocol | Proposed for PI review: represent neighbourhood public charging by a separate uncontrolled ElaadNL Set B of public `cp` profiles with native public car/van mix, fixed `simulated_year = 2030`, `cp_capacity_kw = 22`, 100 profiles per batch, initial candidate `M = 1000` from seeds `150001`-`150901`, and held-out `H = 200` from seeds `151201` and `151301`. The generator unit remains one public charge point profile; the documentation states that two public charge points share one pole connection, so pole-level aggregation or asset reporting must preserve that two-charge-point convention rather than relabel the profile member as a pole. Public profiles would be sampled later under EV-003/EV-005 with finite-library size `M` reported separately from Monte Carlo count `N`, and with the within-realization replacement rule still pending until cohort sizes and E3.S2a adequacy criteria are resolved. This proposal does not authorize generation, public smart charging, integrated net-load/congestion/event analysis, or any claim that `M = 1000` is sufficient. | Public charging blocks executable EV adoption scenarios because EV-004 fixed only the home profile class while EV-007/A-014 now expose public charge-point counts. The 22 kW recommendation matches a public AC charge-point interpretation but remains the main PI check because the local-count unit and the generator's two-charge-points-per-pole convention must align before generation. | `reports/e2_s2_public_profile_decision_packet.md`; `data/metadata/elaad_profiles/B_public_vancar_cp_y2030_decision_packet.json`; ElaadNL `Documentatie Laadprofielengenerator`, 10 November 2025, pp. 5-7, 10, 13-14; EV-002; EV-003; EV-005; EV-007. | proposed | -- |
| EV-008A | 2026-07-21 | Proposed public Set B amendment choice | Proposed for PI review: after D-012, choose between (1) approving EV-008's 22 kW public Set B as an explicit future/upper-capacity convention, with 12 API calls (`M = 1000` candidate from seeds `150001`-`150901` and `H = 200` held-out from seeds `151201`, `151301`), or (2) amending EV-008 to a capacity-stratified public Set B with 11, 13, 15, and 22 kW AC classes, 14 planned API calls, candidate `M = 1000` from seeds `152001`-`152901`, and held-out `H = 200` from seeds `153201`, `153301`, `153401`, and `153501` if 50-profile held-out requests are accepted. Agent C recommends option 2 because D-012 supports the charge-point/EVSE/connector-like unit but shows 22 kW is not the unique current Alkmaar representative capacity. Either option keeps public profiles uncontrolled, `profile_type = cp`, `location_type = public`, native public car/van mix, fixed `simulated_year = 2030`, ignored raw/processed storage, per-seed checkpoints, member traceability by capacity class, batch seed, returned index, request checksum, and output checksums, and no claim that `M = 1000` is sufficient. This proposal does not authorize public Set B generation, public smart charging, held-out adequacy use, integrated analysis, or manuscript results. | D-012 adds current NDW/DOT-NL Alkmaar evidence after EV-008 was drafted. A single 22 kW library is operationally simple but must be framed as a convention; capacity stratification better preserves observed current AC heterogeneity at modest extra API/checkpoint complexity. | `reports/e2_s2_ev008_public_profile_amendment_packet.md`; `data/metadata/elaad_profiles/B_public_vancar_cp_y2030_amendment_packet.json`; `reports/e2_s2_ndw_public_charging_inventory.md`; D-012; EV-002; EV-003; EV-005; EV-008. | proposed | -- |
