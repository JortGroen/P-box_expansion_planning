...
| E2-S3-HP-SCALING-RETRIEVAL-ROUTE | 2026-07-22 | HP-001 Alkmaar public-source retrieval/checksum route | Proposed: bind the HP-001 local scaling route to a public-source-only retrieval and checksum workflow for Alkmaar municipality `GM0361`. The route records CBS StatLine 85035NED as the proposed SFH/MFH dwelling-stock denominator source, PBL Startanalyse aardgasvrije buurten 2025 Alkmaar as the proposed local heat-demand/pathway/suitability evidence source after schema inspection, and CBS StatLine 85523NED as national/current heat-pump context only. Future retrievals must write raw public files under ignored `data/raw/hp_scaling/`, persist source-by-source checkpoints and SHA-256 metadata under `data/metadata/hp_scaling/`, and keep local heat demand, suitability/pathway evidence, and unsigned 2035 HP adoption separate before any value proposal. | This step converts the previous source/proxy clarification into an auditable execution route without using confidential thesis values or silently authorizing annual TWh inputs. It preserves SFH/MFH and space/DHW traceability for the four HP-001 components and keeps Startanalyse suitability/pathway evidence from becoming adoption evidence by implication. The route does not retrieve files, select checksums, approve D-013, calculate annual TWh values, approve 2035 HP adoption, sign D-004, run paired-weather acceptance, run net-load/event/`P(E)`/threshold/capacity-screen analysis, or produce manuscript numbers. | `reports/e2_s3_hp_scaling_retrieval_route.md`; `data/get_hp_scaling.py`; D-013; HP-001; D-003; D-004; WEATHER-001; `reports/e2_s3_hp_local_scaling_source_proxy_clarification.md`. | proposed route; values unsigned | -- |
