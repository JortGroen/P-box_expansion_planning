...
| E2-S3-HP001-COMPONENT-OUTPUT-RUNNER-READINESS | 2026-07-24 | HP-001 component-output runner/preflight scaffold | Proposed: add a deterministic HP-side runner that consumes the component-output readiness manifest and writes a blocker/output manifest before IC-1 can load HP component artifacts. The runner checks final HP approval IDs, rejects stale placeholder approval strings, requires concrete D-003/D-004/D-013 source artifact path/checksum/provenance records, verifies HP/PV WEATHER-001 identity equality, preserves SFH/MFH space/DHW component traceability, blocks accepted-status claims in this scaffold, and records request/input/code identity. A signed synthetic fixture path can emit one tiny IC-1-compatible HP NPZ/manifest for loader tests only. | Automates the HP integration boundary so future real HP output cannot appear merely because a file path exists. Current main emits a blocker manifest only; real component-output generation remains blocked until annual value/adoption/service fractions, A-016, D-004 paired-weather acceptance, and cold-spell tolerances are all signed and concrete artifact checksums exist. This row does not approve annual HP TWh values, 2035 adoption/electrification/service fractions, final D-004 paired-weather or cold-spell acceptance, net-load/event/`P(E)`/capacity-screen analysis, threshold runs, manuscript numbers, or probability results. | `data/get_hp_scaling.py`; `data/metadata/hp_scaling/hp001_component_output_runner_readiness_blocker.json`; `src/hp_model.py`; `tests/test_data_sources.py`; `reports/e2_s3_hp001_component_output_runner_readiness.md`; HP-001; D-003; D-004; D-013; WEATHER-001; A-016. | proposed runner scaffold; real HP outputs blocked | -- |
