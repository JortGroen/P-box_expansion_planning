...
|---|---|---|---|---|---|---|---|
| E3-S2B-PRERUN-GATE | 2026-07-24 | E3.S2b pre-run gate scaffold | Proposed: before the future-layer capacity/domain screen can launch, the A-owned pre-run gate must verify checksum-backed capacity provenance, explicit pending total-versus-firm convention status, accepted component-output manifests whose node-axis and array-shape contract matches the current IC-1 loader, A-016 scenario-consistency evidence, and downstream A-013/G2/capacity-convention blockers. Multi-node component-output artifacts are fail-closed until A adds a compatible loader or C emits per-node loadable manifests. | Prevents metadata wrappers, unsigned values, missing checksums, or spatially collapsed pseudo-node artifacts from becoming executable screen inputs. | This PR; src/contracts/net_load.py; reports/E3_S2B_INTEGRATED_PRERUN_READINESS.md. | proposed | -- |
...
| D014-PV-FIRST-EXPERIMENT-VALUE-DECISION-PACKET | 2026-07-24 | D-014 first-experiment PV value-decision packet | Proposed: use `data/metadata/weather_pv/d014_pv_first_experiment_value_decision_packet.json` as the concise PI value-decision support packet before executable first-experiment PV generation. The packet consumes checksummed capacity value-choice, orientation/tilt value-choice, PV-PARAM conversion source-choice, first-experiment approval, and executable preflight guard metadata, then narrows concrete options and approval keys for statistical orientation/tilt distribution, pvlib-style POA versus direct-GHI fallback, performance/loss treatment, temperature/clipping treatment, capacity convention, node allocation, and A-016 scenario consistency. | Turns the merged approval packets into one reviewable value-decision surface while preserving fail-closed governance. It recommends a lightweight statistical-orientation/tilt route with a signed conversion treatment after PI review, but does not approve any PV capacity value, II3050 growth factor, capacity convention, orientation/tilt bin/weight, PR/loss/temperature/clipping value, pvlib/POA/direct-GHI formula, node allocation, PV output, net-load/event analysis, `P(E)`, threshold run, capacity screen, manuscript number, or final paired HP/PV acceptance. | `data/get_pv_capacity.py`; `data/metadata/weather_pv/d014_pv_first_experiment_value_decision_packet.json`; `reports/e2_s4_d014_pv_first_experiment_value_decision_packet.md`; D-014; PV-CAP-001; PV-ORIENT-001; PV-PARAM-001; A-016; WEATHER-001. | proposed packet; values and executable PV blocked | pending PI review |
| D014-PV-FIRST-EXPERIMENT-VALUE-APPROVAL-PACKET | 2026-07-24 | D-014 first-experiment PV value-approval packet | Proposed: use `data/metadata/weather_pv/d014_pv_first_experiment_value_approval_packet.json` as the concise PI approval packet for the remaining first-experiment PV value route before executable PV generation. The packet consumes checksummed value-decision, capacity value-choice, capacity approval-template, orientation/tilt value-choice, PV-PARAM conversion source-choice, and component-output scaffold metadata, then records explicit signoff choices for the Alkmaar CBS 85005NED anchor row/period/sector/field and DC/AC convention, II3050 2035 scenario/growth denominator under A-016, statistical orientation/tilt values, PV conversion, reactive-power policy, manifest path/checksum policy, and node allocation. | Converts the proposed value-decision surface into one machine-readable approval checklist that future PV generation must satisfy before emitting arrays. It keeps KNMI as the realized WEATHER-001 path, PVGIS as qualitative sanity/provenance only, and PV-ORIENT-001 statistical-only scope intact. It does not approve any PV capacity value, II3050 growth factor, capacity convention, orientation/tilt bin/weight, PR/loss/temperature/clipping value, conversion formula, reactive-power convention, component-output path policy, node allocation, PV output, net-load/event analysis, `P(E)`, threshold run, capacity screen, manuscript number, roof/building/3DBAG/PV-map geometry, or final paired HP/PV acceptance. | `data/get_pv_capacity.py`; `data/metadata/weather_pv/d014_pv_first_experiment_value_approval_packet.json`; `reports/e2_s4_d014_pv_first_experiment_value_approval_packet.md`; D-014; PV-CAP-001; PV-ORIENT-001; PV-PARAM-001; A-016; WEATHER-001. | proposed packet; values and executable PV blocked | pending PI review |
| D014-PV-COMPONENT-OUTPUT-ARTIFACT-SCAFFOLD | 2026-07-24 | D-014 PV component-output artifact scaffold | Proposed: use `data/metadata/weather_pv/d014_pv_component_output_artifact_scaffold.json` as the metadata-only scaffold and blocker manifest for future PV component-output artifact generation. The packet records the IC-1-compatible manifest/NPZ schema, PV export sign convention, checksum/config identity inputs, checkpoint/resume behavior, and approval keys needed before a future signed PV run may emit component-output arrays. Current committed D-014/PV packets remain rejected because capacity, orientation/tilt values, PV-PARAM conversion, allocation, A-016, final paired acceptance, and reactive-power policy are unsigned. | Prepares deterministic output wiring without crossing into Agent A-owned generic assembly code or silently generating PV values from proposed packets. It does not approve or compute PV capacity, growth factor, orientation/tilt weights, conversion formula, loss/temperature/clipping values, reactive-power policy, node allocation, PV output, net-load/event analysis, `P(E)`, threshold run, capacity screen, manuscript number, roof/building/3DBAG/PV-map workflow, or final paired HP/PV acceptance. | `data/get_pv_capacity.py`; `data/metadata/weather_pv/d014_pv_component_output_artifact_scaffold.json`; `src/pv_model.py`; `reports/e2_s4_d014_pv_component_output_artifact_scaffold.md`; D-014; PV-CAP-001; PV-ORIENT-001; PV-PARAM-001; A-016; WEATHER-001. | proposed scaffold; component-output generation blocked | pending PI review |
| D014-PV-COMPONENT-OUTPUT-RUNNER-PREFLIGHT | 2026-07-24 | D-014 PV component-output runner preflight | Proposed: use `data/metadata/weather_pv/d014_pv_component_output_runner_preflight.json` and `src.pv_model.build_pv_component_output_runner_preflight` as the fail-closed runner boundary before PV component-output arrays can be produced. The preflight consumes the merged D-014 value-approval packet and component-output scaffold, requires signed capacity/growth, statistical orientation/tilt, PV conversion/loss/temperature/clipping, node allocation, reactive-power, manifest path/checksum, A-016 scenario-consistency, and final paired HP/PV weather-acceptance artifacts, and emits a blocker manifest while those artifacts are absent. Synthetic-fixture output is allowed only for tests with explicit fixture flags and IC-1 loader validation. | Converts the proposed PV output scaffold into deterministic automation so future wiring cannot accidentally turn unsigned PV choices into arrays. It preserves the PV-ORIENT-001 statistical-only first-experiment scope and rejects roof/building/3DBAG/PV-map geometry hooks. This row does not approve any PV value, formula, allocation, reactive-power policy, real component-output artifact, net-load/event analysis, `P(E)`, threshold run, capacity screen, manuscript number, or final paired HP/PV acceptance. | `src/pv_model.py`; `data/get_pv_capacity.py`; `data/metadata/weather_pv/d014_pv_component_output_runner_preflight.json`; `reports/e2_s4_d014_pv_component_output_runner_preflight.md`; D-014; PV-CAP-001; PV-ORIENT-001; PV-PARAM-001; A-016; WEATHER-001. | proposed runner/preflight; real PV output blocked | pending PI review |
...
| E2-S3-HP001-VALUE-BINDING-DECISION-CANDIDATES | 2026-07-24 | HP-001 local annual value-binding decision candidates | Proposed: add a deterministic D-013 value-candidate extractor and blocker packet for HP-001 local annual scaling. The extractor verifies ignored CBS/PBL raw artifacts against the D-013 retrieval manifest before computing candidates from PBL `Referentie_2030`, `I11_woningequivalenten [Woning]`, approved inferred indicators `H23_Vraag_RV_w` and `H24_Vraag_TW_w` with diagnostic `H22_Vraag_totaal_w`, GJ/year-to-TWh/year conversion by division by `3,600,000`, and CBS 85035NED SFH/MFH count shares. In worktrees without the ignored raw artifacts, the committed metadata packet fails closed with source-artifact blockers and emits no values. | Makes the next PI approval auditable by separating verified source extraction, candidate equations, SFH/MFH plus space/DHW traceability, and unsigned 2035 service-fraction options from executable HP profile generation. The candidate binding record remains rejected by the HP adapter until all remaining annual value-binding approval IDs are signed and A-016, D-004 paired-weather acceptance, and cold-spell tolerances are separately resolved. This row does not approve annual HP TWh values, 2035 adoption/electrification/service fractions, D-004 final paired/cold-spell acceptance, net-load/event/`P(E)`/capacity-screen analysis, threshold runs, manuscript numbers, or probability results. | `data/get_hp_scaling.py`; `data/metadata/hp_scaling/hp001_alkmaar_gm0361_value_binding_decision_candidates_blocker.json`; `reports/e2_s3_hp001_value_binding_decision_candidates.md`; `tests/test_data_sources.py`; D-013; D013-PBL-MAPPING; A-015; HP-001; A-016; D-004. | proposed packet; executable annual values unsigned | -- |
...
| E2-S3-HP001-PROFILE-REBUILD-PREFLIGHT | 2026-07-24 | HP-001 profile artifact rebuild/checksum preflight template and runner scaffold | Proposed: define a fail-closed metadata preflight and deterministic runner before any future HP-001 profile artifact rebuild/checksum run. The preflight requires signed final HP approval IDs, explicit D-003 When2Heat, D-004 WEATHER-001 member, and D-013 value-binding source artifact path/checksum/provenance records, HP/PV paired-weather identity equality, 35,040 quarter-hour output targets, checksum-manifest targets, and no unresolved blocker IDs before the rebuild can run. The runner consumes either the committed template packet or a future direct preflight manifest, records the input checksum, blocker IDs, validator, git/code identity, and non-claims, and stops without profile generation when blockers remain. | Keeps future HP artifact generation from starting on unsigned annual values, unsigned 2035 adoption/electrification/service fractions, missing D-004 paired/cold-spell acceptance, missing source artifacts/checksums, stale placeholder approval strings, or mismatched HP/PV weather identity. This row does not approve annual HP TWh values, 2035 adoption/electrification/service fractions, D-004 final paired-weather or cold-spell acceptance, a real profile rebuild, net-load/event/`P(E)`/capacity-screen analysis, threshold runs, manuscript numbers, or probability results. | `reports/e2_s3_hp001_profile_rebuild_preflight.md`; `data/metadata/hp_scaling/hp001_profile_artifact_rebuild_preflight_template.json`; `data/metadata/hp_scaling/hp001_profile_rebuild_runner_blocker_manifest.json`; `src/hp_model.py`; `data/get_hp_scaling.py`; HP-001; D-003; D-004; D-013; WEATHER-001; A-016. | proposed preflight template and runner scaffold; executable HP artifacts unsigned | -- |
...
## D014-PV-COMPONENT-OUTPUT-RUNNER-PREFLIGHT - PV Component-Output Runner Preflight - 2026-07-24 - proposed

### Proposed Runner Boundary

Use `src.pv_model.build_pv_component_output_runner_preflight` and the committed `data/metadata/weather_pv/d014_pv_component_output_runner_preflight.json` as the deterministic fail-closed boundary before any PV component-output array is produced. The runner consumes the D-014 first-experiment value-approval packet and PV component-output scaffold, then requires checksum-backed signed artifacts for PV capacity and II3050 growth, statistical orientation/tilt distribution, PV conversion/loss/temperature/clipping treatment, node allocation and normalization, reactive-power policy, component-output manifest path/checksum policy, A-016 scenario consistency, and final paired HP/PV weather acceptance. With current main, those artifacts are absent, so the runner emits a blocker manifest and no arrays.

### Boundary

Synthetic-fixture output may be written only in tests with explicit `synthetic_fixture` status and provenance, signed-fixture approval IDs, safe repository-relative paths, and IC-1 loader validation. Real output requires accepted artifacts without stale/template approval tokens, source/checksum metadata, exact paired weather identity, node allocation, and the PV-ORIENT-001 statistical-only scope. The runner rejects roof, building, location-level, 3DBAG, PV-map, and per-roof geometry hooks in the first-experiment path. It does not approve or produce PV capacity, II3050 growth, orientation/tilt values, conversion formulas, losses, temperature or clipping values, reactive-power policy, allocation, real PV output, net-load/event analysis, `P(E)`, threshold analysis, capacity screens, manuscript numbers, or final paired HP/PV acceptance.

### Rationale

This runner/preflight turns the metadata scaffold into executable guardrail code without crossing into scientific execution. It lets future integration call one PV-owned boundary and receive either a checksum-backed component-output manifest from signed inputs or a concise blocker manifest from unsigned/proposed inputs.
...
## D014-PV-FIRST-EXPERIMENT-VALUE-DECISION-PACKET - First-Experiment PV Value-Decision Packet - 2026-07-24 - proposed

### Proposed Packet

Use `data/metadata/weather_pv/d014_pv_first_experiment_value_decision_packet.json` as the concise PI value-decision support packet before executable first-experiment PV generation. The packet consumes checksummed D-014 capacity value-choice, PV-ORIENT-001 orientation/tilt value-choice, PV-PARAM conversion source-choice, first-experiment approval, and executable preflight guard metadata. It narrows the remaining PI choices into explicit options and approval keys for statistical orientation/tilt source, bins, representative angles, weights, and capacity-weighting convention; pvlib-style statistical plane-of-array conversion versus a direct-GHI fallback; performance/loss treatment; temperature and clipping treatment; capacity convention; node allocation; and A-016 scenario consistency.

### Boundary

This packet is a recommendation and review surface only. It does not approve any PV capacity value, II3050 growth factor, capacity convention, orientation/tilt bin or weight, performance ratio, loss value, temperature coefficient, clipping rule, conversion formula, node allocation, PV generation, net-load/event analysis, `P(E)`, threshold analysis, capacity screens, manuscript numbers, or final paired HP/PV acceptance. PVGIS remains qualitative sanity/provenance context only, and roof/building/location-specific 3DBAG or PV-map geometry remains deferred until after the first experiment.

### Rationale

The previous D-014 packets each isolate one gate. This packet gives the PI one compact checklist for selecting the values and formulas needed to make first-experiment PV executable, while keeping every unsigned value fail-closed for downstream code and integration.
## D014-PV-FIRST-EXPERIMENT-VALUE-APPROVAL-PACKET - First-Experiment PV Value-Approval Packet - 2026-07-24 - proposed

### Proposed Packet

Use `data/metadata/weather_pv/d014_pv_first_experiment_value_approval_packet.json` as the PI-facing approval packet for the remaining first-experiment PV value route before executable PV generation. The packet consumes checksummed D-014 value-decision, capacity value-choice, capacity approval-template, PV-ORIENT-001 value-choice, PV-PARAM conversion source-choice, and component-output scaffold metadata. It turns those inputs into explicit signoff choices for the CBS 85005NED Alkmaar `GM0361` anchor row, period, sector/category, capacity field and DC/AC convention; the II3050 2035 scenario column, denominator, formula and A-016 scenario consistency link; the statistical orientation/tilt source, bins, representative angles, weights and weighting convention; the PV conversion route and PVGIS/KNMI boundary; a PV reactive-power policy; repository-relative manifest path/checksum policy; and node allocation/normalization.

### Boundary

This packet is a proposed approval surface only. It authorizes no executable PV generation, no real component-output arrays, and no component-output manifest. It requires signed successor artifacts for capacity, II3050 growth, A-016 consistency, capacity convention, statistical orientation/tilt values, PV-PARAM conversion/loss/temperature/clipping treatment, node allocation, reactive-power policy, manifest path/checksum policy, and final paired HP/PV acceptance. PVGIS remains qualitative sanity/provenance context only, KNMI remains the realized WEATHER-001 weather path, and roof/building/location-specific 3DBAG or PV-map geometry remains deferred until after the first experiment.

### Rationale

The project now has separate proposed packets for capacity, scenario growth, orientation/tilt, conversion, and future output artifacts. This approval packet gives the PI one compact signature checklist and gives future code a deterministic fail-closed gate: if the required approval keys are missing, the only valid output is a blocker manifest, not PV arrays or scientific numbers.
## D014-PV-COMPONENT-OUTPUT-ARTIFACT-SCAFFOLD - PV Component-Output Artifact Scaffold - 2026-07-24 - proposed

### Proposed Scaffold

Use `data/metadata/weather_pv/d014_pv_component_output_artifact_scaffold.json` as the metadata-only scaffold for future first-experiment PV component-output artifact generation. The scaffold records the IC-1-compatible manifest and NPZ fields expected from a PV component output, including `artifact_id`, `artifact_status`, `kind = pv`, node/member/source/calendar identity, `shared_weather_driver_id`, repository-relative `array_path`, `array_sha256`, and provenance. It also records that future PV generation exported into IC-1 uses negative `p_kw`, while nonnegative `generation_kw` remains an internal PV diagnostic. A future reactive-power policy, or an explicit `q_kvar = 0` convention, remains unsigned.
...
The project needs the PV-owned side of the future component-output bridge before PI signoff, but unsigned values must not flow into IC-1. A fail-closed scaffold lets reviewers inspect the planned manifest shape, checksums, weather-driver identity, and checkpoint behavior while preserving the hard boundary against real PV generation, net-load/event analysis, `P(E)`, capacity screens, or manuscript results.
