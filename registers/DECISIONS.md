...
| E2-S3-COLD-SPELL-ACCEPTANCE-DESIGN | 2026-07-21 | HP cold-spell and paired-weather acceptance design | Proposed: before final integrated HP/PV source acceptance, a future acceptance report should verify that When2Heat-derived HP profiles and PV profiles preserve the same shared weather realization, canonical UTC/local calendar, source/provenance, and paired temperature/irradiance fields, then report predeclared cold-spell and temperature-response diagnostics. Numerical pass/fail tolerances remain unsigned and must be approved before the first real acceptance run. | Keeps any HP source/technology boundary separate from final paired-weather acceptance and prevents HP/PV weather consistency from being judged after integrated congestion results are inspected. This row does not set tolerances, run the check, sign D-003 or D-004, approve local annual HP scaling, or authorize net-load, event, `P(E)`, capacity-screen, manuscript, or probability results. | `reports/e2_s3_cold_spell_acceptance_design.md`; HP-001 remains proposed; Q-8 remains open; D-003 and D-004 remain proposed. | proposed acceptance design | -- |
| E2-S3-HP-TECH-SCALING-DECISION-PACKET | 2026-07-21 | HP technology and annual scaling decision packet | Proposed: the PI review packet frames the remaining E2.S3 choices before real HP integration: Dutch When2Heat shape columns, COP/technology columns, optional water-heating inclusion, SFH/MFH/COM class handling, and whether annual thermal scaling comes from When2Heat `heat_demand_*` evidence or from another registered source. It proposes review options only and leaves all scientific choices unsigned. HP-001 records a recommended first-pass source/technology boundary while keeping annual local scaling and paired-weather acceptance open. | Collects previously separate HP source-use, annual-scaling, and cold-spell blockers into one PI-facing decision map so D-003 source use is not silently conflated with technology scenario, annual volume, local downscaling, or paired-weather acceptance. This row does not sign D-003/D-004, set numerical tolerances, run paired-weather acceptance, authorize net-load/event/`P(E)`/capacity/manuscript/probability results, or approve any final HP integration input. | `reports/e2_s3_hp_technology_scaling_decision_packet.md`; D-003 and D-004 remain proposed; HP-001 remains proposed pending explicit PI approval. | proposed packet; HP-001 recommendation pending PI decision | -- |
| HP-001 | 2026-07-21 | Proposed first-pass HP source and technology boundary | Proposed for PI approval: use D-003 When2Heat `2023-07-27` `when2heat.csv` as the internal source for Dutch normalized residential space-heating shapes and COP trajectories. The proposed primary HP layer uses `NL_heat_profile_space_SFH` and `NL_heat_profile_space_MFH` with `NL_COP_ASHP_radiator`, preserving SFH and MFH as separate components until aggregation. Commercial (`COM`) heat and domestic hot-water heat would be excluded from the primary run and may only enter later signed sensitivities. Do not use When2Heat national `heat_demand_*` values as the local 2035 annual scaling by default; retain them as diagnostic/source anchors only. Agent C must propose a separate local annual HP scaling/adoption route before real integrated HP load is used. This proposal would approve D-003 only for internal shape/COP source use under the stated boundary; D-004/Q-8 paired-weather implementation, cold-spell acceptance tolerances, final local scaling, event analysis, `P(E)`, capacity screens, and manuscript results remain blocked until separately signed. | This recommended boundary keeps the first HP implementation scientifically narrow: residential space heat matches the neighbourhood consumption-congestion story, ASHP radiator is transparent and already supported by the retrieved Dutch columns, and excluding water/COM avoids silently adding different end-use boundaries. Separating shape/COP from annual volume prevents country-level When2Heat heat-demand totals from becoming an unreviewed local 2035 adoption assumption. | `reports/e2_s3_hp_technology_scaling_decision_packet.md`; `reports/e2_s3_hp_source_use_pi_packet.md`; `reports/e2_s3_hp_scaling_evidence_packet.md`; `reports/e2_s3_d003_pi_review_packet.md`; D-003 retrieval/checksum metadata. | proposed | -- |
