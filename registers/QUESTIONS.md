...
STATUS: resolved by EV-007 and EV-007A in `registers/DECISIONS.md`; PI approved Option A on 2026-07-21 and then accepted Alkmaar municipality `GM0361` as the representative local proxy with the retrieved 2035 low/middle/high home/public counts. Agent C may use those local totals as executable declared adoption branches and should next materialize A-014 per-node allocations without choosing the final paper branch.
...
STATUS: resolved by WEATHER-001 in `registers/DECISIONS.md`; PI approved Option A on 2026-07-21. Agent C is authorized by ownership policy to implement the neutral shared weather-member contract in `src/weather_model.py` and `tests/test_weather_model.py`; D-004 source acceptance, completeness checks, cold-spell tolerances, paired-weather acceptance results, and integrated analysis remain separate blockers.

## Q-9 - E2.S4 - 2026-07-22 - BLOCKING: yes
CONTEXT: PR #106 proposed `D004-MC-001`, the rule for converting the approved hourly KNMI/PVGIS D-004 source bundle into 15-minute WEATHER-001 members. Latest `origin/main` has the packet but no signed DECISIONS row approving or amending it.
QUESTION: Does the PI approve `D004-MC-001` as proposed, or require an amended construction rule before Agent C implements the D-004 member builder?
OPTIONS: A) Approve `D004-MC-001` as proposed - Agent C implements UTC calendar-year members, Europe/Amsterdam local timestamps derived from UTC, zero-order KNMI temperature expansion, energy-preserving KNMI `Q` to 15-minute GHI, PVGIS as calibration/validation provenance only, and the proposed member IDs.  B) Amend before implementation - PI specifies the calendar basis, hourly-to-15-minute handling, realized irradiance source, provenance fields, or member-ID convention to change.
RECOMMENDATION: A, because it preserves the approved four-file route, keeps KNMI as the realized temperature/GHI weather path, keeps PVGIS from becoming an independently sampled weather realization, and is straightforward to audit under WEATHER-001.
STATUS: resolved by D004-MC-001 in `registers/DECISIONS.md`; PI approved Option A in chat on 2026-07-22. Agent C may implement the D-004 member builder from the approved rule, but final D-004 source acceptance, completeness tolerances, HP/PV paired acceptance, net-load/event analysis, `P(E)`, capacity screens, and manuscript results remain separate blockers.
