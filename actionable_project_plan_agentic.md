...
| **G1 Foundation validated** | end W2 | Approved in `registers/DECISIONS.md`: Tier-1 full-year radial summation is the MC inner-loop evaluator; AC is deterministic/validation only; fixed winter windows rejected; WindowSets retained only for AC validation/diagnostics; no manuscript claim may say "AC infeasible" | Laptop micro-benchmark (E1.S2); profile/window validation (E1.S3, E1.S3b); G1 brief | proceed with E1.S4; complete C1 TimeSeriesCPP benchmark and C2 transformer-headroom memo before G2 |
