...
**T6 — Git discipline.** Agents work on branches (`agent-a/...`), open PRs with a filled self-review checklist (tests pass, invariants pass, manifest attached, registers updated); **only you merge to `main`.**
...
|---|---|---|---|---|
| **G0 Scope freeze** | end W1 | Overload-event definition E (e.g., any 15-min step with transformer loading >100% within the critical window of a planning year); P_crit = 1e-2 (+1e-3 sensitivity); grid codes (SimBench semi-urban primary, urban/CIGRE secondary); weather source (KNMI-driven Dutch data on SimBench topology) | E0 registers initialized; grid inventory report (E1.S1) | freeze / adjust |
...
|---|---|---|---|
| **E0.S1 Repo & environment** | T1 repo skeleton (`src/ tests/ configs/ data/ experiments/ registers/ reports/ paper/`); T2 pinned `environment.yml` (Python 3.12, pandapower 3.x, simbench 1.6.1, lightsim2grid ≥0.9.2, numba, numpy/pandas/scipy, pytest, hypothesis, matplotlib); T3 `Makefile` (`make test / run / figures`); T4 CI (pytest+lint) | Repo with green CI | Fresh-clone install + `make test` passes on your laptop |
...
| **E1.S4 Tier-1 evaluator** | T1 radial downstream summation → transformer loading series; T2 overload-event flag per G0 definition; T3 unit tests vs hand-computed toy feeder | `src/evaluator_sum.py` + tests | Matches hand calculation exactly on toy case |

### E2 — Data & aleatory layer (Owner: C; Weeks 1–3)
| Story | Tasks | Deliverable | Acceptance / Verification |
|---|---|---|---|
| **E2.S1 Data acquisition** | T1 retrieval scripts: ElaadNL open datasets, When2Heat (DOI 10.25832/when2heat), PVGIS/KNMI weather, SimBench profiles; T1b unit-cost extraction from the Cicėnas (2025) thesis appendix (Stedin-confirmed €/MVA and €/km tables); T2 license check; T3 checksums + `DATA_REGISTER` entries | `data/get_*.py` + signed register rows | Every dataset: source, license, checksum, your sign-off |
| **E2.S2 EV model** | T1 fit arrival/energy/SoC distributions from ElaadNL; T2 sampler; T3 QQ-plot validation | `src/ev_model.py` + fit report | QQ plots acceptable; sampler seeded |
