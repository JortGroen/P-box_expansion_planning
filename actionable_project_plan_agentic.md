...
**T11 — Fast integration is fail-closed.** Agents may use stacked PRs or an integration branch to expose wiring problems earlier, but no placeholder may flow into a number. Integrated runners and preflights must reject unsigned values, proposed-only assumptions, missing artifacts, unverified required checksums, unresolved blocker IDs, and unsafe executable tokens such as `TODO`, `TBD`, `placeholder`, `synthetic`, `proposed`, `unsigned`, or `not-approved` unless explicitly allowlisted as non-executable metadata. The required output of an unready integration run is a concise blocker manifest for PI review, not a provisional scientific result.

**Escalation triggers (agent must stop and ask you):** interface change needed; a test cannot pass without changing the spec; unclear data license; a process expected to exceed about 15 minutes lacks a durable resume path; repeated mechanical work would be scaled manually instead of through a tested script/runner; runtime exceeds budget by >2×; an integrated fast path would produce a number from unresolved blockers/placeholders; results contradict a passed Gate; anything requiring a value not in the registers.
